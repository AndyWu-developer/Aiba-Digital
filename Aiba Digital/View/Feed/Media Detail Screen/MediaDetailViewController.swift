...
//    viewDidLayoutSubviews get's called when the direct subviews of my view did layout, but the subviews of that subviews did not layout at that moment, so you can't get the size of them.
    ...
    private func configureCollectionViewDataSource(){
        ...
        let photoCellRegistration = UICollectionView.CellRegistration<PhotoDetailCell,PhotoCellViewModel>(cellNib: UINib(nibName: String(describing: PhotoDetailCell.self), bundle: nil)) { cell, indexPath, viewModel in
            cell.configure(with: viewModel)
            cell.delegate = self
        }
        
        let videoCellRegistration = UICollectionView.CellRegistration<VideoDetailCell,VideoCellViewModel>(cellNib: UINib(nibName: String(describing: VideoDetailCell.self), bundle: nil)) { cell, indexPath, viewModel in
